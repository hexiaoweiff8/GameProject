...
public class GenLuaApi
{
    ...
    [MenuItem("Lua/GenBabeluaApiFile")]
    ...
    {
        System.IO.File.WriteAllText(Application.dataPath + "/Lua/UnityApi.lua", sb.ToString());
        xmlNodeCache = null;
        sb = null; loadedModule = null;
        Debug.Log("生成自定义luaApi成功");
    }
    ...
    private static void GenClass(Type type)
    {
        ...
        foreach (var item in nameToMethods)
        {
            ...
            foreach (MethodInfo method in thisNameMethods)
            {
                ...
                if (isStatic)
                {
                    sb.Append("--" + returnType + " " + type.Name + "." + item.Key + "(");
                }
                else
                {
                    sb.Append("--" + returnType + " " + type.Name + ":" + item.Key + "(");
            ...
            if (isStatic)
            {
                sb.Append("function " + type.Name + "." + item.Key + "() end \r\n\r\n");
            }
            else
            {
                sb.Append("function " + item.Key + "() end \r\n\r\n");
            }
