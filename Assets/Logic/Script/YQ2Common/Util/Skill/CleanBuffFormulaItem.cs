...
/// 净化效果
/// </summary>
class CleanBuffFormulaItem : AbstractFormulaItem
    {
    ...
    public CleanBuffFormulaItem(string[] array)
        ...
        public override IFormula GetFormula(FormulaParamsPacker paramsPacker)
        {
            ...
            IFormula result = new Formula((callback, scope) =>
            {
                ...
                if (myTargetPos == 0)
                {                
                    for (int i = 0; i< member.ClusterData.AllData.BuffInfoList.Count; i++)
                    {
                        if (!member.ClusterData.AllData.BuffInfoList[i].IsCouldNotClear)
                        {
                            member.ClusterData.AllData.BuffInfoList.Remove(member.ClusterData.AllData.BuffInfoList[i]);
                        }                  
                    }
                }
                ...
                {
                    for (int i = 0; i < target.ClusterData.AllData.BuffInfoList.Count; i++)
                    {
                        if (!target.ClusterData.AllData.BuffInfoList[i].IsCouldNotClear)
                        {
                            target.ClusterData.AllData.BuffInfoList.Remove(target.ClusterData.AllData.BuffInfoList[i]);
                        }
                    }
                }
                callback();
