wnd_cangku_ScrollView_controller = {
	...
	_selectedItemIconFrame, -- 记录当前处于选择状态的Item的IconFrame

	_State_InDECOMPOSITION = false,-- 标记当前是否处于装备分解界面

	cstr_SELECTED = 'zhuangbei_zhuangbeikuang_xuanzhong',
	cstr_SELECTED_CB = "zhuangbei_tubiao_pinjieshaixuan",
...
function wnd_cangku_ScrollView_controller:initScrollViewItem()
	...
	Item.cItem = Item.transform:GetChild(2).gameObject
	Item.cEquipment = Item.transform:GetChild(3).gameObject
...
function wnd_cangku_ScrollView_controller:setQualityMark(cangkuItem,Quality)
	if Quality == 1 then
		cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_baisepinzhi")
	elseif Quality == 2 then
		cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_lvsepinzhi")
	elseif Quality == 3 then
		cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_lansepinzhi")
	elseif Quality == 4 then
		cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_zisepinzhi")
	elseif Quality == 5 then
		cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_chengsepinzhi")
	else cangkuItem:setIconFrame("zhuangbei_zhuangbeikuang_hongsesunhuai") end
end
----------------------------------------------------------------
--★Callback
function wnd_cangku_ScrollView_controller:HandleOnItemLoadedHandler(item)
	...
	if cangkuItem.cItem == nil then
		cangkuItem.cItem = cangkuItem.transform:GetChild(2).gameObject
	end
	if cangkuItem.cEquipment == nil then
		cangkuItem.cEquipment = cangkuItem.transform:GetChild(3).gameObject
	end	
	...
	if cangkuItem.Index + 1 < #this.currentItems then -- 索引不在范围内，则显示空物品
		...
		if this.currentItems[cangkuItem.Index+1]["EquipID"] ~= nil then -- 如果是装备
			...
			print("索引："..(cangkuItem.Index+1)..tostring(this.currentItems[cangkuItem.Index+1].selected))
		else -- 如果是道具
	...
	elseif cangkuItem.Index + 1 <= 6 * 5 then
...
function wnd_cangku_ScrollView_controller:HandleOnItemClickedHandler(cangkuItem)
	...
	if cangkuItem.Index + 1 < #this.currentItems then -- 索引在范围内
		...
		if this.currentItems[cangkuItem.Index+1]["EquipID"] ~= nil then
			...
			ctrl:showEquipmentDetailsPanel(this.currentItems[cangkuItem.Index+1])
		else
...
function wnd_cangku_ScrollView_controller:HandleOnItemSelectedHandler(cangkuItem)
	...
	if this._selectedItem ~= nil and
		this._selectedItem ~= cangkuItem and
	 	this._selectedItem.sIconFrame.spriteName ~= view.pItem_empty:GetComponent(typeof(UISprite)).spriteName then
		this._selectedItem:setIconFrame(this._selectedItemIconFrame)
	end

	this._selectedItemIconFrame = cangkuItem.sIconFrame.spriteName

	cangkuItem:setIconFrame(this.cstr_SELECTED) 
...
function wnd_cangku_ScrollView_controller:filterBy(Goods,Maintype)
	...
	this._selectedItemIconFrame = nil
	-- ListView重新加载方法
	if #this.currentItems < 6 * 5 then
		_scrollView:Reload(6 * 5)
	else
	...
	if #this.currentItems ~= 0 then
		...
		if this.currentItems[1]["EquipID"] ~= nil then
			print("显示装备信息")
			ctrl:showEquipmentDetailsPanel(this.currentItems[1])
		else
...
function wnd_cangku_ScrollView_controller:disselectEquipmentByQuality(rarity)
	...
	for i = 1,#this.currentItems do
		if this.currentItems[i].rarity == rarity then
			print('处理品质：'..this.currentItems[i].rarity)
