local leftCard_controller = {}
local _view = require("uiscripts/fight/leftCard/leftCard_view")
local _data = require("uiscripts/fight/leftCard/leftCard_model")

function leftCard_controller:Init(view)
    _view:initView(view)
...
---刷新方法
function leftCard_controller:Refresh()
...
---刷新剩余卡牌的显示
function leftCard_controller:initLeftCardsPanel()
...
---刷新剩余兵力的显示
function leftCard_controller:initLeftMilitaryStrength()
    _view.Left_MS_curLab:GetComponent("UILabel").text = stringUtil:getString(10031)--剩余兵力
    _view.allBingLiLabel:GetComponent("UILabel").text = _data:getLeftMilitaryStrengthValue()
end

---刷新下一张卡牌的显示
function leftCard_controller:initNextCard()
...
---为界面中的按钮添加监听
---
function leftCard_controller:AddListener()
    --牌库显示
