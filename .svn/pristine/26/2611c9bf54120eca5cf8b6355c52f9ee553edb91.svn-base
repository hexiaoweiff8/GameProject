...
//#解决抛出选择事件问题，原版抛出时间做了个分支，两种抛出2选一。
//改成了两种模式同时抛出
...
[ExecuteInEditMode]
[AddComponentMenu("QK/UI/Popup List")]
public class QKUIPopupList : UIWidgetContainer
{
	/// <summary>
	/// Current popup list. Only available during the OnSelectionChange event callback.
	/// </summary>

	static public QKUIPopupList current;
    ...
    const string functionName = "OnSelectionChange";
	...
	public void Set (string value, bool notify = true)
	{
		if (mSelectedItem != value)
		{
			...
			//mSelectedItem = null; //QK wenchuan 完全没必要设置为空啊，会导致弹出列表时高亮条位置不对
		}
	}
	...
	protected void TriggerCallbacks ()
	{
		if (!mExecuting)
		{
            ...
            if ( !string.IsNullOrEmpty(functionName))
			{
				...
				gameObject.SendMessage(functionName, mSelectedItem, SendMessageOptions.DontRequireReceiver);
			}
			current = old;
			mExecuting = false;
		}
	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
            ...
            pos.x = t.localPosition.x;//QK <按钮靠边的时候弹出框位置不对>
