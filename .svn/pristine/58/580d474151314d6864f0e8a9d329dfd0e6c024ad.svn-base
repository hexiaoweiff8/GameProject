...
public class SocketClient
{
    ...
    private void OnReceive(byte[] bytes, int length)
    {
        ...
        {
            int messageLen = mBinaryReader.ReadInt32();
            messageLen = System.Net.IPAddress.NetworkToHostOrder(messageLen);
            if (RemainingBytes() >= messageLen)//如果剩余的字节大于等于读取包头得到的包体长度，说明这条数据传输完毕了可以读取
            {
                MemoryStream ms = new MemoryStream();
                BinaryWriter writer = new BinaryWriter(ms);
                writer.Write(mBinaryReader.ReadBytes((int)messageLen));
                ms.Seek(0, SeekOrigin.Begin);
                OnReceivedMessage(ms);
            }
            else//如果数据传输未完，回退4个字节（这4个字节为包体长度）
            {
                //Back up the position two bytes
                mMemoryStream.Position = mMemoryStream.Position - 4;
                break;
            }
        }
        //Create a new stream with any leftover bytes
        //读取完成后将数据传输未完的字节存入到新的流中等待下次判断是否传输完毕
    ...
    private void OnReceivedMessage(MemoryStream ms)
    {
        ...
        NetworkManager.AddEvent(104, message);
    }
