...
function table.removebyvalue2(array, value)
    local i, max = 1, #array
    while i <= max do
        if array[i] == value then
            table.remove(array, i)
...
function table.upset(array)
    for i = #array, 1, -1 do
        local index = math.random(#array)
        local tempNum = array[i]
        array[i] = array[index]
        array[index] = tempNum
    end
end
...
function string.splitToInt(input, delimiter)
    input = tostring(input)
    delimiter = tostring(delimiter)
    if (delimiter == '') then return false end
    local pos, arr = 0, {}
...
function GetRemainTime(remainTime)
    ...
    if remainTime < 86400 then --显示时分秒
        ...
        return string.format("%02d", _remainHour) .. "h " .. string.format("%02d", _remianMin) .. "m " .. string.format("%02d", _remainSec) .. "s"
    else --显示天时分
        ...
        return tostring(_remainDay) .. "d " .. string.format("%02d", _remainHour) .. "h " .. string.format("%02d", _remianMin) .. "m"
    end

end

function startTimer(time, tickFunc, endFunc, name)
    local timer = TimeTicker()
    timer:Start(time)
    timer.OnTick = tickFunc
    timer.OnEnd = endFunc
    if name then
        allTimeTickerTb[name] = timer
    end
    return timer
end
